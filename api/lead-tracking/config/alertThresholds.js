const ALERT_THRESHOLDS = {
  IMMEDIATE: {
    ...
    recipients: {
      sms: process.env.EMERGENCY_PHONE_NUMBERS ? process.env.EMERGENCY_PHONE_NUMBERS.split(',').map(s => s.trim().replace(/\r|\n/g, '')).filter(s => s) : [],
      email: process.env.EMERGENCY_EMAILS ? process.env.EMERGENCY_EMAILS.split(',').map(e => e.trim().replace(/\r|\n/g, '')).filter(e => e && e.includes('@')) : []
    },
  ...
  HIGH_PRIORITY: {
    ...
    recipients: {
      email: process.env.HIGH_PRIORITY_EMAILS ? process.env.HIGH_PRIORITY_EMAILS.split(',').map(e => e.trim().replace(/\r|\n/g, '')).filter(e => e && e.includes('@')) : []
    },
  ...
  STANDARD: {
    ...
    recipients: {
      email: process.env.STANDARD_EMAILS ? process.env.STANDARD_EMAILS.split(',').map(e => e.trim().replace(/\r|\n/g, '')).filter(e => e && e.includes('@')) : []
    },
